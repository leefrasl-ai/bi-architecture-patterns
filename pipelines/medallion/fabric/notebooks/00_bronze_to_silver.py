# ============================================
# BRONZE TO SILVER TRANSFORMATION - CORRECTED
...
#   ✓ Uses DataFrame unions instead of SQL temp views
#   ✓ Creates temp views immediately before use
#   ✓ Explicit schema for logging to avoid type inference
#   ✓ All temp view persistence issues resolved
#
# ENHANCEMENTS:
...
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.silver_dim_patient")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.silver_dim_provider")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.silver_dim_facility")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.silver_dim_payer")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.silver_dim_clinic")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.silver_fact_visit")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.quarantine_patients")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.quarantine_visits")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.data_quality_errors_silver")
spark.sql("DROP TABLE IF EXISTS lh_healthcare_silver.dbo.etl_batch_log_silver")
...
spark.sql("""
    CREATE TABLE IF NOT EXISTS lh_healthcare_silver.dbo.etl_batch_log_silver (
        batch_id STRING,
        table_name STRING,
...
spark.sql("""
    CREATE TABLE IF NOT EXISTS lh_healthcare_silver.dbo.data_quality_errors_silver (
        error_id STRING,
        batch_id STRING,
        table_name STRING,
        error_type STRING,
...
spark.sql("""
    CREATE TABLE IF NOT EXISTS lh_healthcare_silver.dbo.quarantine_patients (
        patient_id STRING,
...
spark.sql("""
    CREATE TABLE IF NOT EXISTS lh_healthcare_silver.dbo.quarantine_visits (
        visit_id STRING,
        source_system STRING,
        patient_id STRING,
        provider_id STRING,
...
def log_batch(table_name, status, rows_processed=0, rows_inserted=0, rows_rejected=0, error_msg=None):
    ...
    log_df.write.format("delta").mode("append").saveAsTable("lh_healthcare_silver.dbo.etl_batch_log_silver")
...
try:
    ...
    patients_a = spark.sql("""
        SELECT *, 'HSA' as source_system FROM lh_healthcare_bronze.dbo.bronze_dimpatient
    """)
    patients_a.createOrReplaceTempView("patients_a_raw")

    patients_b = spark.sql("""
        SELECT *, 'UCN' as source_system FROM lh_healthcare_bronze.dbo.bronze_uc_patients
    """)
    ...
    if rejected_count > 0:
        ...
        invalid_patients.write.format("delta").mode("append").saveAsTable("lh_healthcare_silver.dbo.quarantine_patients")
        ...
        for row in invalid_patients.limit(100).collect():
            ...
            spark.createDataFrame(error_data, ["error_id", "batch_id", "table_name", "error_type", 
                                               ...
                                               "created_timestamp"]).write.format("delta").mode("append").saveAsTable("lh_healthcare_silver.dbo.data_quality_errors_silver")
    else:
        print("✓ All patient records passed validation")


    # STEP 6: Save valid records to Silver
    valid_patients.write.format("delta").mode("overwrite").saveAsTable("lh_healthcare_silver.dbo.silver_dim_patient")
...
try:
    ...
    providers_a = spark.sql("""
        SELECT *, 'HSA' as source_system FROM lh_healthcare_bronze.dbo.bronze_dimprovider
    """)

    providers_b = spark.sql("""
        SELECT *, 'UCN' as source_system FROM lh_healthcare_bronze.dbo.bronze_uc_clinicians
    """)
    ...
    providers_a = spark.sql("""
        ...
        FROM lh_healthcare_bronze.dbo.bronze_dimprovider
    """)
    ...
    providers_b = spark.sql("""
        ...
        FROM lh_healthcare_bronze.dbo.bronze_uc_clinicians
    """)
    ...
    providers_silver.write.format("delta").mode("overwrite").saveAsTable("lh_healthcare_silver.dbo.silver_dim_provider")
...
try:
    ...
    facilities_a = spark.sql("""
        SELECT 
            ...
            State as state,
            CAST(NULL AS STRING) as postal_code
        FROM lh_healthcare_bronze.dbo.bronze_dimfacility
    """)
    ...
    facilities_b = spark.sql("""
        ...
        FROM lh_healthcare_bronze.dbo.bronze_uc_sites
    """)
    ...
    facilities_silver.write.format("delta").mode("overwrite").saveAsTable("lh_healthcare_silver.dbo.silver_dim_facility")
...
try:
    ...
    payers_a = spark.sql("""
        ...
        FROM lh_healthcare_bronze.dbo.bronze_dimpayer
    """)
    ...
    payers_b = spark.sql("""
        ...
        FROM lh_healthcare_bronze.dbo.bronze_uc_payers
    """)
    ...
    # STEP 5: Save to Silver
    payers_silver.write.format("delta").mode("overwrite").saveAsTable("lh_healthcare_silver.dbo.silver_dim_payer")
...
try:
    ...
    clinics_silver = spark.sql("""
        SELECT 
            ...
            CURRENT_TIMESTAMP() as modified_timestamp,
            '{batch_id}' as etl_batch_id
        FROM lh_healthcare_bronze.dbo.bronze_dimclinic
    """.format(batch_id=etl_batch_id))
    ...
    clinics_silver.write.format("delta").mode("overwrite").saveAsTable("lh_healthcare_silver.dbo.silver_dim_clinic")
...
try:
    ...
    visits_a = spark.sql("""
        ...
        FROM lh_healthcare_bronze.dbo.bronze_factappointment
    """)
    ...
    visits_b = spark.sql("""
        SELECT 
            ...
            CASE 
                ...
                THEN TO_TIMESTAMP(CONCAT(DATE(Visit_DateTime), ' ', Checkin_Time))
                ELSE NULL 
            END as checkin_datetime,
            CASE 
                WHEN CheckOut_Time IS NOT NULL AND CheckOut_Time != '' 
                THEN TO_TIMESTAMP(CONCAT(DATE(Visit_DateTime), ' ', CheckOut_Time))
                ELSE NULL 
            ...
            END as copay_amt
        FROM lh_healthcare_bronze.dbo.bronze_uc_visits
    """)
    ...
    if rejected_count > 0:
        ...
        invalid_visits.write.format("delta").mode("append").saveAsTable("lh_healthcare_silver.dbo.quarantine_visits")
    else:
    ...
    orphan_visits = spark.sql("""
        ...
        LEFT JOIN lh_healthcare_silver.dbo.silver_dim_patient p
    ...
    # STEP 6: Save valid records to Silver
    valid_visits.write.format("delta").mode("overwrite").saveAsTable("lh_healthcare_silver.dbo.silver_fact_visit")
...
dq_summary = spark.sql("""
    ...
    FROM lh_healthcare_silver.dbo.data_quality_errors_silver
...
print("\nQuarantine Summary:")
try:
    q_patients = spark.sql("SELECT COUNT(*) FROM lh_healthcare_silver.dbo.quarantine_patients WHERE batch_id = '{}'".format(etl_batch_id)).collect()[0][0]
    q_visits = spark.sql("SELECT COUNT(*) FROM lh_healthcare_silver.dbo.quarantine_visits WHERE batch_id = '{}'".format(etl_batch_id)).collect()[0][0]
